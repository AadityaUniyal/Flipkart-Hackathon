...
# Helper function to compute target encoding within a cross-validation scheme
def get_oof_te_kfold(df, n_splits=5, m=20):
    ...
    for tr_idx, val_idx in kf.split(df):
        ...
        # geohash_te
        stats_geo = tr_fold.groupby("geohash")["demand"].agg(["mean", "count"])
        te_geo = ((stats_geo["count"] * stats_geo["mean"] + m * global_mean) / (stats_geo["count"] + m)).to_dict()
        df.iloc[val_idx, df.columns.get_loc("geohash_te")] = df.iloc[val_idx]["geohash"].map(te_geo)
        
        # geo_hour_demand
        stats_hour = tr_fold.groupby(["geohash", "hour"])["demand"].mean().reset_index(name="val")
        val_slice = df.iloc[val_idx][["geohash", "hour"]]
        val_merged = val_slice.merge(stats_hour, on=["geohash", "hour"], how="left")
        val_merged["val"] = val_merged["val"].fillna(pd.Series(df.iloc[val_idx]["geohash"].map(te_geo).fillna(global_mean).values, index=val_merged.index))
        df.iloc[val_idx, df.columns.get_loc("geo_hour_demand")] = val_merged["val"].values

        # geo_weather_demand
        stats_weather = tr_fold.groupby(["geohash", "Weather"])["demand"].mean().reset_index(name="val")
        val_slice = df.iloc[val_idx][["geohash", "Weather"]]
        val_merged = val_slice.merge(stats_weather, on=["geohash", "Weather"], how="left")
        val_merged["val"] = val_merged["val"].fillna(pd.Series(df.iloc[val_idx]["geohash"].map(te_geo).fillna(global_mean).values, index=val_merged.index))
        df.iloc[val_idx, df.columns.get_loc("geo_weather_demand")] = val_merged["val"].values

    # Fill NaNs
    df["geohash_te"] = df["geohash_te"].fillna(global_mean)
    df["geo_hour_demand"] = df["geo_hour_demand"].fillna(df["geohash_te"])
    df["geo_weather_demand"] = df["geo_weather_demand"].fillna(df["geohash_te"])
    return df
...
# Filter to Day 48 only for training/validation (since Day 49 is very small)
...
# 7. Compute target encodings using 5-fold OOF on Day 48
train_df = get_oof_te_kfold(train_df, n_splits=5)
...
print("Evaluating models with 5-Fold Cross Validation on Day 48...")
