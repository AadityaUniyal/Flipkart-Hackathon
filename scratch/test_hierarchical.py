...
def add_spatial_temporal_features(train_df, test_df):
    ...
    for col in ["geohash_te", "loc_te", "loc_hour_te", "loc_weather_te"]:
        train_df[col] = np.nan
    ...
    for tr_idx, val_idx in kf.split(train_df):
        ...
        # Map to fold and fillna
        train_df.iloc[val_idx, train_df.columns.get_loc("geohash_te")] = train_df.iloc[val_idx]["geohash"].map(geo_te)
        
        # 2. loc_te (smooth towards geohash_te)
        # We need the geohash_te values for the training fold to compute the loc_te smoothing
        tr_fold_geo_te = tr_fold["geohash"].map(geo_te).fillna(global_mean)
        loc_stats = tr_fold.groupby("loc_key")["demand"].agg(["mean", "count"])
        # Map geohash_te to loc_stats to smooth properly
        # Find corresponding geohash for each loc_key
        ...
        # 3. loc_hour_te (smooth towards loc_te)
        hour_stats = tr_fold.groupby(["loc_key", "hour"])["demand"].agg(["mean", "count"]).reset_index()
        # Map loc_te
        hour_stats["loc_te"] = hour_stats["loc_key"].map(loc_te).fillna(global_mean)
        hour_stats["te"] = (hour_stats["count"] * hour_stats["mean"] + m3 * hour_stats["loc_te"]) / (hour_stats["count"] + m3)
        
        val_slice = train_df.iloc[val_idx][["loc_key", "hour"]]
        val_merged = val_slice.merge(hour_stats[["loc_key", "hour", "te"]], on=["loc_key", "hour"], how="left")
        # Use loc_te as fallback
        ...
        # 4. loc_weather_te (smooth towards loc_te)
        weather_stats = tr_fold.groupby(["loc_key", "Weather"])["demand"].agg(["mean", "count"]).reset_index()
        weather_stats["loc_te"] = weather_stats["loc_key"].map(loc_te).fillna(global_mean)
        weather_stats["te"] = (weather_stats["count"] * weather_stats["mean"] + m4 * weather_stats["loc_te"]) / (weather_stats["count"] + m4)
        
        val_slice = train_df.iloc[val_idx][["loc_key", "Weather"]]
        val_merged = val_slice.merge(weather_stats[["loc_key", "Weather", "te"]], on=["loc_key", "Weather"], how="left")
        val_merged["te"] = val_merged["te"].fillna(pd.Series(train_df.iloc[val_idx]["loc_key"].map(loc_te).fillna(global_mean).values, index=val_merged.index))
    ...
    hour_stats = train_df.groupby(["loc_key", "hour"])["demand"].agg(["mean", "count"]).reset_index()
    hour_stats["loc_te"] = hour_stats["loc_key"].map(loc_te).fillna(global_mean)
    hour_stats["te"] = (hour_stats["count"] * hour_stats["mean"] + m3 * hour_stats["loc_te"]) / (hour_stats["count"] + m3)
    
    test_merged = test_df[["loc_key", "hour"]].merge(hour_stats[["loc_key", "hour", "te"]], on=["loc_key", "hour"], how="left")
    test_df["loc_hour_te"] = test_merged["te"].fillna(test_df["loc_te"]).values

    weather_stats = train_df.groupby(["loc_key", "Weather"])["demand"].agg(["mean", "count"]).reset_index()
    weather_stats["loc_te"] = weather_stats["loc_key"].map(loc_te).fillna(global_mean)
    weather_stats["te"] = (weather_stats["count"] * weather_stats["mean"] + m4 * weather_stats["loc_te"]) / (weather_stats["count"] + m4)
    
    test_merged = test_df[["loc_key", "Weather"]].merge(weather_stats[["loc_key", "Weather", "te"]], on=["loc_key", "Weather"], how="left")
...
print("Training models with hierarchical spatial-temporal target encoding...")
